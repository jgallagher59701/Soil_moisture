...
// TODO Find a better way to pass this into the device.
#define SENSOR_ID "1"
...
#define SOIL_SENSOR_POWER A0
#define SOIL_SENSOR_ADDR 0x36

// TODO Make this the only status light.
...
#if USE_SD
// SD library and log file object
...
// cycles the led at 'pin' ('freq' Hz) n 'times' to indicate various things
// If 'times' is zero, blink forever (i.e., never return because an error was
// detected.
...
void lora_setup() {
  ...
  } IO(Serial.print(F("Set Freq to: "))); IO(Serial.println(RF95_FREQ));
...
void DS3231_setup() {
  ...
  RTC.writeSqwPinMode(DS3231_BBSQW);
...
// Initialize the SD card for data logging. If there's no card, blink the
// operational status LED and continue on.
// TODO Should it be a fatal error to not have a SD card?
#if USE_SD

void writeHeader() {
  file.println(F("<date-time>,<packetnum>,<Vcc>,<rssi>,<snr>,<temp>"));
  file.sync();
}
...
void sd_card_setup() {
  if (!sd.begin(SD_CARD_CS, SD_SCK_MHZ(50))) {
    sd.initErrorHalt(F("SD card library failure: "));
  }
  ...
  if (!file.open(file_name, O_WRONLY | O_CREAT | O_EXCL)) {
    error("file.open");
  }

  IO(Serial.print(F("Logging to: "))); IO(Serial.println(file_name));
...
void send_packet(char *time_stamp, float temp) {
  ...
  if (rf95.waitAvailableTimeout(1000)) {
    ...
    if (rf95.recv(buf, &len)) {
      ...
      IO(Serial.print(F("Got reply: "))); IO(Serial.println((char*)buf)); IO(Serial.print(F("RSSI: "))); IO(Serial.println(rssi, DEC));
    } else {
      rssi = 0;
      snr = 0;
...
void log_measurement() {
  IO(Serial.print(F("logging packet: "))); IO(Serial.println(packet));

  if ((file.println(packet) != -1) && (file.sync() != -1)) {
    blink_times(OPERATIONAL_STATUS, 2, 5);
  } else {
...
#define SOIL_SENSOR_WAKEUP_DELAY 2000 // ms
float soil_sensor_temp;
uint16_t soil_capread;

// Read the soil capacitance and temperature. Assume that the sensor was
// just powered up, so perform a reset on the capacitive sensor component.
void soil_sensor_measurement() {
  // Read a value to reset capacitive sensor
  soil_capread = ss.touchRead(0);
  IO(Serial.print(F("Capacitive after wake up: ")));
  IO(Serial.print(soil_capread));

  // A 2s wait seems needed, although the begin() method only uses 0.5s
  delay(SOIL_SENSOR_WAKEUP_DELAY);

  soil_sensor_temp = ss.getTemp();
  soil_capread = ss.touchRead(0);

  IO(Serial.print(F("Temperature: ")));
  IO(Serial.print(soil_sensor_temp));
  IO(Serial.println(F("C")));
  IO(Serial.print(F("Capacitive: ")));
  IO(Serial.println(soil_capread));
  IO(Serial.flush());   // flush() i/o before sleep
}
...
void soil_sensor_setup() {
  if (!ss.begin(SOIL_SENSOR_ADDR)) {
    IO(Serial.println("ERROR! seesaw not found"));
    blink_times(OPERATIONAL_STATUS, 10, 0);
  } else {
    IO(Serial.print("seesaw started! version: "));
    IO(Serial.println(ss.getVersion(), HEX));
    blink_times(OPERATIONAL_STATUS, 2, 20);
  }
}
...
void setup() {
  ...
  // Power on the stemma soil sensor
...
void loop() {
  ...
  // This should be moved into the wake_up callback. jhrg 4/29/19
