...
const char data_file_name[] = "/data_log_1.txt";
...
void SD_card_setup()
{
  ...
  if (!SD.begin(SD_CARD_CS)) {
    IO(Serial.println("Card failed, or not present"));
    blink_times(OPERATIONAL_STATUS, 10 /* Hz */, 100); // 10 Hz blink
  }
  else {
    IO(Serial.println("card initialized."));
  #if 0
    // Open the data file in append mode
    data_file = SD.open(data_file_name, FILE_WRITE);
    if (data_file) {
      IO(Serial.println("opened "));
      IO(Serial.println(data_file_name));
    }
    else {
      IO(Serial.println("error opening "));
      IO(Serial.println(data_file_name));
      // blink_times(OPERATIONAL_STATUS, 10 /* Hz */, 100); // 10 Hz blink
    }
    #endif
...
// \param info Null terminated string to write to the log file.
#if USE_SD
void log_measurement(char *info)
{
  SD_card_select();
...
void loop()
{
  ...
  char *packet = send_packet(iso8601_date_time(RTC.now()), RTC.getTemp());
  // log_measurement(packet);
